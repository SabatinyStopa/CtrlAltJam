...
public class Ally : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		if (transform.localScale.x * m_Rigidbody2D.velocity.x > 0 && !m_FacingRight && life > 0) Flip();
		else if (transform.localScale.x * m_Rigidbody2D.velocity.x < 0 && m_FacingRight && life > 0) Flip();
	}
	...
	public void MeleeAttack()
	{
		...
		for (int i = 0; i < collidersEnemies.Length; i++)
		{
			...
			{
				collidersEnemies[i].gameObject.GetComponent<PlayerController>().ApplyDamage(2f, transform.position);
			}
		}
